...
*/
    ...
    else if ( state == Email && responseLine == "235" )
    {
...
void  Mail::browse()
{
    ...
    ui->attatchment->setText( fileListString );
...
void   Mail::sendMail()
{
    Mail* mail = new Mail("mohamedskander.laouiti@esprit.tn",ui->password->text(), "mail.gmail.com");
    connect(mail, SIGNAL(status(QString)), this, SLOT(mailSent(QString)));

   /* if( !files.isEmpty() )
        mail->sendMail("mohamedskander.laouiti@esprit.tn", ui->recipient->text() , ui->subject->text(),ui->msg->toPlainText(), files );
    else
        mail->sendMail("mohamedskander.laouiti@esprit.tn", ui->recipient->text() , ui->subject->text(),ui->msg->toPlainText());
*/
}
